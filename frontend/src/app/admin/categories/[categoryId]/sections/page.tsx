  ...
  const handleSave = async () => {
    ...
    try {
      if (editingSection) {
        ...
        await sectionsApi.create({ name: formName, categoryId: Number(categoryId) });
      }
    ...
    <div>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: categoryName || '...', href: '/admin/categories' },
          { label: "Bo'limlar" },
        ]}
        action={
          <Button
                ...
                {sections.map((section) => (
                  ...
                  >
                    <td className="px-4 py-4 text-slate-700 font-medium">
                      <Link 
                        href={`/admin/categories/${categoryId}/sections/${section.id}/lessons`}
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-md bg-white">
          ...
          <div className="grid gap-4 py-4">
            {/* Biriktirilgan kurs — pre-filled, read-only */}
            <div className="flex flex-col gap-1.5">
              <Label className="text-sm font-semibold text-slate-700">Biriktirilgan kurs</Label>
              <Select value={categoryId} disabled>
                <SelectTrigger className="h-11 rounded-xl border-slate-200 bg-slate-50 text-slate-600">
                  <SelectValue>{categoryName || 'Kategoriya'}</SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value={categoryId}>{categoryName}</SelectItem>
                </SelectContent>
              </Select>
            </div>
