...
export default function CategoryLessonsPage({
  ...
  params: Promise<{ categoryId: string; sectionId: string }>;
}) {
  const { categoryId, sectionId } = React.use(params);
  ...
  const loadData = () => {
    ...
    Promise.all([
      ...
      .then(([lessonsRes, sectionRes, catsRes]) => {
            ...
            <table className="w-full text-[13px] text-left">
              ...
              <tbody>
                {lessons.map((lesson) => (
                  ...
                  >
                    <td className="px-4 py-4 text-slate-700 font-medium">{categoryName}</td>
                    ...
                    <td className="px-4 py-4">
                      {lesson.introVideo ? (
                         <button 
                           onClick={() => setPreviewVideo(`http://localhost:3001/uploads/videos/${lesson.introVideo}`)}
                    ...
                    <td className="px-4 py-4">
                      <Link href={`/admin/categories/${categoryId}/sections/${sectionId}/lessons/${lesson.id}/materials`}>
