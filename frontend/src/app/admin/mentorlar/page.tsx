...
import { Eye, Edit2, Trash2, EyeOff } from 'lucide-react';
...
export default function MentorlarPage() {
  ...
  const openEditModal = (mentor: Mentor) => {
    ...
    setFormData({ name: mentor.name, phone: mentor.phone, password: '' });
    setIsModalOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    setIsSaving(true);
    try {
      await mentorsApi.delete(selectedId.toString());
  ...
  const columns = [
    ...
    { 
      ...
      render: (row: Mentor) => <span className="text-slate-500">{row.phone}</span>
    },
    { 
      key: 'create_at', 
      header: 'Yaratilgan vaqt',
      render: (row: Mentor) => <span className="text-slate-500">{row.create_at ? new Date(row.create_at).toLocaleString('ru-RU') : '-'}</span>
    },
    { 
      key: 'role', 
      header: 'Rol',
      render: (row: Mentor) => <span className="text-slate-600 capitalize">Mentor</span>
    },
    { 
      key: 'password', 
      header: 'Parol',
  ...
  return (
    <div>
      ...
      />
      <DataTable columns={columns} data={data} isLoading={loading} emptyTitle="Mentorlar topilmadi" />
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-md bg-white">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="flex flex-col gap-2">
              <Label htmlFor="password">Parol {editingMentor && "(o'zgartirish uchun)"}</Label>
              <Input
                id="password"
                type="password"
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="sm:max-w-sm bg-white">
          ...
          <div className="py-4 text-slate-600">
            Haqiqatan ham bu mentorni o'chirmoqchimisiz? Bu amalni ortga qaytarib bo'lmaydi.
          </div>
          <DialogFooter>
